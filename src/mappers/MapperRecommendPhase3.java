	...
	protected void map(LongWritable key, Text value, Context context)
		...
		try {
			if (valueRead.startsWith(Protocols.ACCURACY_STARTS_WITH)) {
				...
				// String[] userAndFriendsList =
				// valueRead.substring(Protocols.COLD_START_STARTS_WITH.length()).split(Protocols.VALUE_SEPARATOR);
				//
				// String[] friends = userAndFriendsList[1].substring(1,
				// userAndFriendsList[1].length() -
				// 1).split(Protocols.FRIEND_LIST_SEPARATOR);
				//
				// // FloatWritable userFloat = new
				// FloatWritable(Float.parseFloat(userAndFriendsList[0]));
				// int userID = Integer.parseInt(userAndFriendsList[0]);
				// for(String eachUser : friends)
				// {
				// if(!eachUser.isEmpty())
				// {
				// context.write(new Text(Protocols.COLD_START_USERS_STARTS_WITH
				// + eachUser.trim()), new FloatWritable(userID));
				// }
				// }
